...
begin
  if (hLibmng = 0) then begin
     ...
     {procedure}
     {****************************************************************************}
     {procedure}@_jpeg_CreateDecompress:= GetProcAddress(hLibmng, 'jpeg_CreateDecompress');
     {procedure}@_jpeg_stdio_src:= GetProcAddress(hLibmng, 'jpeg_stdio_src');
     {procedure}@_jpeg_read_header:= GetProcAddress(hLibmng, 'jpeg_read_header');
     {procedure}@_jpeg_calc_output_dimensions:= GetProcAddress(hLibmng, 'jpeg_calc_output_dimensions');
     @_jpeg_start_decompress:= GetProcAddress(hLibmng, 'jpeg_start_decompress');
     @_jpeg_read_scanlines:= GetProcAddress(hLibmng, 'jpeg_read_scanlines');
     @_jpeg_finish_decompress:= GetProcAddress(hLibmng, 'jpeg_finish_decompress');
     {procedure}@_jpeg_destroy_decompress:= GetProcAddress(hLibmng, 'jpeg_destroy_decompress');
     @_jpeg_has_multiple_scans:= GetProcAddress(hLibmng, 'jpeg_has_multiple_scans');
     @_jpeg_consume_input:= GetProcAddress(hLibmng, 'jpeg_consume_input');
		 @_jpeg_start_output:= GetProcAddress(hLibmng, 'jpeg_start_output');
     @_jpeg_finish_output:= GetProcAddress(hLibmng, 'jpeg_finish_output');
     {procedure}@_jpeg_destroy:= GetProcAddress(hLibmng, 'jpeg_destroy');
     {procedure}@_jpeg_CreateCompress:= GetProcAddress(hLibmng, 'jpeg_CreateCompress');
     {procedure}@_jpeg_stdio_dest:= GetProcAddress(hLibmng, 'jpeg_stdio_dest');
     {procedure}@_jpeg_set_defaults:= GetProcAddress(hLibmng, 'jpeg_set_defaults');
     {procedure}@_jpeg_set_quality:= GetProcAddress(hLibmng, 'jpeg_set_quality');
     {procedure}@_jpeg_set_colorspace:= GetProcAddress(hLibmng, 'jpeg_set_colorspace');
     {procedure}@_jpeg_simple_progression:= GetProcAddress(hLibmng, 'jpeg_simple_progression');
     {procedure}@_jpeg_start_compress:= GetProcAddress(hLibmng, 'jpeg_start_compress');
     {procedure}@_jpeg_write_scanlines:= GetProcAddress(hLibmng, 'jpeg_write_scanlines');
     {procedure}@_jpeg_finish_compress:= GetProcAddress(hLibmng, 'jpeg_finish_compress');
     @_jpeg_resync_to_restart:= GetProcAddress(hLibmng, 'jpeg_resync_to_restart');
  end;
  Result:= True;
end;
